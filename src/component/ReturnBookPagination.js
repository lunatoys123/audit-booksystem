...
const ReturnBookPagination = (props) => {
  ...
  const NextPage = () => {
    ...
    if (page < totalPage) {
      dispatch(fetchNextPage({ page, limit, ComputerNo }));
    }
  };
  ...
  const PreviousPage = () => {
    const ComputerNo = props.ComputerNo;
    if (page > 1) {
      dispatch(fetchPreviousPage({ page, limit, ComputerNo }));
    }
  };

  const goToTargetPage = (event) => {
    const ComputerNo = props.ComputerNo;
    const targetPage = event.target.innerHTML;

    if (targetPage <= totalPage) {
      dispatch(fetchToTargetPage({ page: targetPage, limit, ComputerNo }));
    }
  };

  const goToFirstPage = () => {
    const ComputerNo = props.ComputerNo;
    dispatch(fetchToTargetPage({ page: 1, limit, ComputerNo }));
  };

  const goToLastPage = () => {
    const ComputerNo = props.ComputerNo;
    dispatch(fetchToTargetPage({ page: totalPage, limit, ComputerNo }));
  };
        ...
        <div className="mx-auto">
          ...
          >
            ...
            <button
              ...
              onClick={(event) => goToTargetPage(event)}
            >
              {page < 3 ? 1 : page - 2}
            </button>
            <button
...
export default ReturnBookPagination;
